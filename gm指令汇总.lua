...
注意: V1.5版本的战斗测试主要通过测试UI进行(按V键打开)
      以下命令用于快速调试和特殊测试场景

命令: /battletest
说明: 快速开始一场测试战斗(自动生成兵种)
参数: 无
用法: /battletest
功能:
    - 在Attack位置1生成1个1级Noob
    - 在Defense位置1生成1个1级Noob
    - 自动开始战斗
成功输出:
    战斗测试开始! 战斗ID: 1
...
说明: 在战斗测试区域生成兵种
参数:
    team     - 队伍(必填)，可用值: attack, defense
    unitId   - 兵种ID(必填)，可用值: Noob, Rookie
    level    - 等级(必填)，范围: 1-3
    position - 位置(必填)，范围: 1-5
用法示例:
    /spawnunit attack Noob 1 1      # 在攻击方位置1生成1级Noob
    /spawnunit defense Rookie 2 3   # 在防守方位置3生成2级Rookie
成功输出:
    已在Attack位置1生成 Lv.1 Noob
...
5. 拖动相同等级兵种到另一个相同兵种上测试合成  # V1.4新增
6. 查看兵种头顶等级是否更新为Lv.2  # V1.4新增
7. /clearplaced             # 清除已放置 V1.2新增
8. /clearunits              # 清空测试
9. /listunits               # 验证清空
10. /addunit InvalidUnit    # 测试错误处理

【战斗系统测试流程 - V1.5新增】
1. 按V键打开战斗测试UI
2. 选择队伍(Attack)、兵种(Noob)、位置(1)、等级(1)
3. 点击"生成兵种"按钮
4. 选择队伍(Defense)、兵种(Noob)、位置(1)、等级(1)
5. 点击"生成兵种"按钮
6. 点击"开始战斗"按钮
7. 观察兵种自动寻找目标、移动、攻击
8. 等待一方全灭，战斗结束
...
【V1.5】 2025-10-31
- 新增: 战斗系统 - 兵种AI、目标寻找、移动攻击
- 新增: 战斗管理器 - 支持多玩家同时战斗
- 新增: 伤害计算系统 - 攻击力、血量管理
- 新增: 弹道系统 - 远程兵种弹道追踪
- 新增: 战斗测试工具 - 测试UI(按V键打开)
- 新增: /battletest 命令 - 快速开始测试战斗
- 新增: /spawnunit 命令 - 生成测试兵种
- 新增: /startbattle 命令 - 开始战斗
- 新增: /stopbattle 命令 - 停止战斗
- 新增: /clearbattle 命令 - 清理战场
- 新增: 兵种配置 - 攻击距离、移动速度、弹道速度

【V1.4】 2025-10-30
- 新增: 兵种属性系统 - 生命值、攻击力、攻击速度
- 新增: 兵种等级系统 - 1-3级，等级系数计算
- 新增: 兵种合成系统 - 拖动相同等级兵种合成升级
- 新增: 等级显示 - 模型头顶显示Lv.X或Lv.Max
- 说明: 合成操作通过拖动完成，无需额外命令
