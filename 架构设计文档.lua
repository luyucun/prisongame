...
│
└── (注: Config配置文件已迁移到ReplicatedStorage/Config下)
...
├── Config/                    (配置文件 - V1.5.1迁移到此)
│   ├── GameConfig             (游戏配置 ModuleScript - 存储游戏常量和配置)
│   ├── UnitConfig             (兵种配置 ModuleScript - 存储兵种属性 V1.5更新 + V1.5.1扩展)
│   ├── PlacementConfig        (放置配置 ModuleScript - 存储放置系统配置 V1.2)
│   └── BattleConfig           (战斗配置 ModuleScript - 存储战斗系统配置 V1.5 + V1.5.1扩展)
│
├── Events/                    (远程事件)
...
│   │   ├── UpdatePosition     (位置更新 RemoteEvent V1.4.1)
│   │   └── UpdateResponse     (位置更新响应 RemoteEvent V1.4.1)
...
│       ├── BattleStateUpdate  (战斗状态更新 RemoteEvent)
│       ├── UnitDeath          (兵种死亡通知 BindableEvent - 服务端内部)
│       └── ShowDamageNumber   (显示伤害数字 RemoteEvent V1.5.1新增)
│
└── Modules/                   (共享模块)
