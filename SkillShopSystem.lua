...
local function OnPurchaseSkill(player, skillId)
	local success, result = pcall(function()
		...
		if not addSuccess then
			-- 失败时回滚金币
			CurrencySystem.AddCoins(player, validPrice, "购买失败退款: " .. skillId, { ApplyVipBonus = false })
			PurchaseLocks[player] = false
		...
		if not stockDeductSuccess then
			-- 回滚金币和技能
			CurrencySystem.AddCoins(player, validPrice, "库存扣除失败退款: " .. skillId, { ApplyVipBonus = false })
