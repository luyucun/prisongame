...
另外我们会定义兵种等级：兵种基础等级为1级，可以提升兵种等级，最高等级为5级

兵种的生命值与攻击力会随着等级提升而提升，在等级提升后，数值会有相应的提升
具体计算逻辑是：
兵种属性=1级时属性*等级*等级系数

1级的等级系数是1，2级是1.2，3级是1.8，4级是2.5，5级是3.3
也就是比如1级攻击力是100，2级就是100*2*1.2=240，3级就是100*3*1.8=540
...
也就是说当达到最高级时，固定就是这个表现了，比如2个4级合了一个5级，但是5级是最高级，这时候就强制使用最高级的颜色表现而不是使用第5级的表现
...
3.关于击杀兵种获得金币：
    1）每个兵种配置基础击杀金币数值
    2）杀死这个兵时获得金币
    3）1级兵就是配置的基础值，2级兵就是金币基础数*2，3级兵就是金币基础数*3，4级兵就是金币基础数*4，5级兵就是金币基础数*5
...
1.我们游戏中有战斗力系统，玩家的兵种数是战斗力数值，买新的兵种或者合成不同等级的兵可以提升战斗力
2.我们需要在玩家战斗力数值发生变化的时候，为玩家播放一个战斗力数值变化的动效
3.具体逻辑是，当玩家的战斗力数值发生变化，需要立刻将StarterGui - TipsSystem - Power的visible属性改成True
4.Power是一个Frame，其下有个子节点叫PowerText，是一个TextLabel，用于显示我们的战斗力数值
5.战斗力数值格式显示为：原战斗力+变化的战斗力数值+上/下箭头，比如100+20↑，代表我原来是100，加了20，其中原战斗力固定为白色，+XXX，如果是增加了战斗力，就显示为绿色，如果战力力减少就是红色，格式-XXX，如果加战斗力箭头就是↑，如果战斗力减少就是↓
6.战斗力数值需要个变化动画，类似金币现在的变化的数值动画，只有变化的数值播放动画，原来的数值不播放
7.当战斗力数值变化动画完，再把StarterGui - TipsSystem - Power的visible属性改成false
8.如果快速发生连续变化，则每次有新的战斗力变化，就立刻隐藏之前的动画，立刻再播新的动画


需求文档V4.4.2  胜利结算界面效果优化

1.我们当前的胜利结算界面是有动画展示逻辑，但是缺少相关的本场战斗的信息展示，需要补充这块的逻辑展示
2.在StarterGui - Victory下增加了叫Bg的Frame，当结算界面打开时，也要同步把Bg 的Visible属性改成True，其中Bg下面有个子节点叫ProgressBg，这里的结构跟distance的结构一致，也是ProgressBg下有个PlayerIcon，代表玩家头像，ProgressBg是进度，当Bg显示出来时，这里要定格显示最终的玩家的本场关卡的进度
3.在StarterGui - Victory - Information - CashNum是我们新增的textlabel，用于显示本次挑战总共获得的金币数值，格式是：+$XXXX,XXXX 就是这次获得的总数值
4.在StarterGui - Victory - Information - DistanceText，是我新加的关于本次挑战关卡进度的文本，格式是：Distance to Escape: 80%，假如我10关打了6关，就是剩下4关，所以显示Distance to Escape: 40%。百分比数值是剩余的百分比。Distance to Escape: 用默认白色，百分比，如果是剩下0%到20%，就显示绿色，如果是21%到50%用黄色，51%到100%用红色
5.动画播放顺序是：先播现在的两个图片的动画，然后播放CashNum动画，然后播放DistanceText和ProgressBg动画，这两个同时播放，最后再播放确认按钮，然后玩家点击确认按钮后一起全部再隐藏

需求文档V4.4.3 新徽章添加

我们需要添加一个徽章：2832522491747257
获得方式是：玩家第一次通关第一章的时候，发放这个徽章（也就是首次通关第一章的所有关卡时发放）


需求文档V4.5 新手对话系统

概述：
我要在游戏中添加一套与NPC的对话系统，玩家通过与NPC对话来学习游戏如何游玩以及打开兵种商店

详细规则：

一、触发
1.玩家进入游戏后会被分配到一个家园中，这是我们已经实现的逻辑，每个家园中都有一个NPC叫KeepShoper01，这是我们要触发对话的主要NPC（注意我们只开发关于KeepShoper01的对话，KeepShoper02相关的关于技能商店的相关触发逻辑还是保持不变的）
2.玩家靠近KeepShoper01，自动触发打开对话框（注意之前是靠近KeepShoper01自动打开兵种商店，我们这里修改为触发打开对话框列表）
3.打开对话框列表即：将StarterGui - Talk - Bg的visible属性改成True，玩家离开范围，自动将对话框关闭

二、对话列表
1.StarterGui - Talk - Bg - ScrollingFrame - ChatTemplate是我们的对话选项模板，用于加载我们的Text文本，在设定中，我们会设定多个Text文本，也就是玩家要与NPC进行对话的选项，玩家可以从中选择一个，触发对应的对话
2.生成对话时，需要读取我们的列表中的所有Text文本，根据ID进行排序，依次生成对话选项，其中ChatTemplate - TalkText是一个textlabel，用于承载Text的文本
3.ChatTemplate由于是个模板，所以默认的Visible属性是false，在生成时，需要把复制出来的对话选项的Visible属性改成True
4.我们会有不同的对话选项，每个选项也不是每次都出现，有些选项在触发某些条件后就会消失。所以我们的列表生成时，需要在TalkText的文本前加上[X],X代表排序，比如生成了3个对话选项，第一个选项是“i want buy something”，那这个选项生成出来后就是[1]i want buy something
5.玩家鼠标移动到对应的选项上时，需要给这个选项一个放大效果，鼠标移走，再把这个选项大小恢复正常。
6.玩家点击这个选项时，需要有一个按下去的变小的效果，送手后恢复正常并触发下一步


三、对话加载
1.我们的对话选项有以下这些类型：
    a.常驻：每次玩家打开对话列表，都会显示出来
    b.一次性对话：玩家只要点击过该选项，以后再次打开对话列表界面，这个对话就不再出现
    c.需要条件触发：也就是玩家一定要达成某个条件后，才会触发这个对话选项

2.注意：我上述说的3种对话选项，某个对话是可以同时满足多个类型的，比如类型b和类型c就可同时满足，比如需要玩家通关某一章，触发对话时才会触发的对话，然后玩家点击该对话后，这个对话就永久消失了

3.对话排序：我会给所有的对话一个排序id，比如1就排在2上面，2就排在3上面，当对话列表生成时，需要根据这个对话排序id来对对话选项进行排序
 
四、对话出现条件：
1.目前我们需要设定以下几个对话出现类型条件：
    a.默认出现，只要玩家触发对话列表生成，就会生成该选项
    b.需要完成某个指定Id的对话，然后再次打开对话，才会显示出来的对话选项
    c.需要通关第一章，并且再次打开对话列表后，才会加载出来的对话选项

五、对话消失条件
1.我们需要设定以下几个对话选项消失条件：
    a.只要玩家点击过该选项，并成功触发了该对话的下一步对话内容，再次打开对话列表时，这个对话就再也不会出现了。注意如果只是关闭了下一步的对话返回了这一步，还是会保留显示的，一定要是关闭了对话选项列表，然后再打开，才会触发消失，注意这里主要就是给一次性对话用的


六、对话流程
1.我们在选择了某个对话选项后，会有以下几种结果：
    a.触发对话内容展示的对话框
    b.触发打开商店页面
    c.关闭对话列表界面

2.当类型是触发对话内容展示时的逻辑是：
    a.先将对话选项列表隐藏（注意只是隐藏不是移除），然后将对下一步的对话内容生成出来（即把StarterGui - TalkBottom - Bg的visible属性改成true），在Bg下有个叫TalkText的textlabel，用于展示对话内容，Bg下有个叫Arrow的imagelabel，需要给这个imagelabel做一个缓缓上下小幅度浮动的循环动画，这个代表提示玩家这个对话可以点击
    b.如果这个对话是这个对话选项中的最后一条对话（唯一一条也算最后一条），则点击这个对话框（注意要赋予点击属性，这目前只是由extlabel以及Frame组成的），立刻隐藏这个对话框，并且再把之前隐藏的对话选项列表显示出来，注意，不是要重新生成对话列表，只是把隐藏的显示出来
    c.已经点击过的对话选项列表，可以再次点击触发对话

3.当类型是触发打开商店页面时，点击选项可以直接关闭对话框，然后打开兵种商店页面，然后关闭商店页面后，也不需要再把对话框列表显示出来，要想打开需要先离开再靠近才再次生成或者按下交互键才打开

4.当类型是关闭对话列表界面时，点击后直接关闭对话框列表，想要再打开也是需要先离开再靠近才会打开，或者按交互键才能再打开

5.需要注意的是：如果玩家点击关闭商店页面或者点击关闭对话列表的选项导致了对话框不再显示后，需要立刻在KeepShoper01身上显示出交互按钮（roblox默认的ProximityPrompt逻辑，我在每个KeepShoper01下的HumanoidRootPart下都加入了ProximityPrompt，玩家点击交互按钮可以再次立刻打开对话列表，注意再次打开时走的是重新生成对话列表的逻辑

七、对话奖励
1.我们需要在某些对话完成后，给玩家发放一些奖励，目前设定中就是：给玩家发放金币，暂时只做这一个类型，后面可能会进行拓展


详细对话需求：
ID	排序	对话选项类型	对话出现条件	出现条件参数	选项文本	点击效果	对话文本1	对话文本2	金币奖励数量
1001	1	一次性对话	默认出现	None	这里发生了什么	触发对话内容	你被绑架了	我现在给你200金币	200
1002	2	一次性对话	通关第一章	None	为什么我还没逃离	触发对话内容	因为还有其他监狱	None	None
1003	3	常驻	完成指定id	1001	购买囚犯	打开商店界面	None	None	None
1004	4	常驻	完成指定id	1001	如何提升实力	触发对话内容	1.不断挑战获取金币，以及领取挂机金币奖励，然后购买各种兵	2.合成更高等级的兵	None
10005	5	常驻	完成指定id	1001	离开	关闭对话列表	None	None	None

注意以上所有涉及到ui出现隐藏的部分，我都希望有一个合适的出现或消失动画，不要太生硬


需求文档V4.5.1   关于挂机奖励的领取

1.移除当前引导中玩家离线后，再次登录时的引导玩家去邮箱领取挂机金币的引导
2.在玩家离线后再次登录时，需要主动触发弹框弹出，让玩家点击领取，具体规则见下面：

关于挂机金币弹框的规则：

1.当玩家离线后再次登录时，如果玩家未领取的挂机金币数值大于等于1，那么需要主动弹出挂机金币领取弹框，也就是主动把StarterGui - IdleEarningGui - Bg的Visible属性改成True，注意出现的时候给个出现的小动效，比如从小变大这种平滑变化动画
2.玩家点击StarterGui - IdleEarningGui - Bg - Title - CloseButton这个按钮，可以关闭弹框，注意，关闭弹框只是关闭，不要触发给玩家领金币的操作，点这个只是意味着玩家暂时先不领取，关闭弹框时也要给打开时类似的关闭动效
3.StarterGui - IdleEarningGui - Bg - CurrentTime - Time是一个textlabel，用于显示玩家当前挂机金币已经积累的总时间，格式是XX:YY，代表小时：分钟，比如03:25,就是玩家已经积累了3小时25分钟的挂机金币奖励，每次打开界面时都需要更新这个数值，并且界面在打开的情况下如果时间发生了变化，也要及时更新
    a.关于挂机时间的累计显示有个细节需要注意：我们的挂机奖励积累时间是有上限的，也就是说比如挂机时间上限16个小时，那这里最多显示到16:00就不再变化了，因为已经不再累计了，这里要注意，也就是达到上限就不加时间了

4.StarterGui - IdleEarningGui - Bg - Claim是一个按钮，玩家点击后，可以直接领取当前已经积累的挂机金币，点击领取后发放金币，播放金币数字变化的动画并且同步关闭弹框，另外也要同步播放我们游戏中领金币时的音效
    a.StarterGui - IdleEarningGui - Bg - Claim - CashNum是一个textlabel，用于显示玩家当前的已经积累的金币数值，格式是$XXXXX,注意如果打开界面过程中数值发生了变化也要同步更新这个数值

5.StarterGui - IdleEarningGui - Bg - Claim10是一个按钮，玩家点击后，直接触发对开发者商品：3487946200的购买，这个开发者商品作用是：直接为玩家发放10倍的当前数值的挂机金币，比如当前积累了100金币，那购买商品后玩家实际可以获得1000金币
    a.玩家点击按钮触发开发者商品购买，如果购买成功则发放金币并关闭界面
    b.玩家如果触发商品购买但是又没购买，关闭了购买弹框，则不关闭这个界面，保持原状即可
    c.StarterGui - IdleEarningGui - Bg - Claim10 - CashNum是一个textlabel，用于显示玩家当前挂机金币累计数值乘10后的数值，比如挂机金币积累了100，那这里的数值就显示是$1000

6.玩家点击CloseButton/Claim/Claim10这三个按钮时，都需要有对应的点击效果，也就是按下去缩小松手恢复正常

7.StarterGui - IdleEarningGui - Bg - Claim10是一个imageButton，我加入了渐变子节点UIGradient用来控制渐变，现在的这个渐变是静态的，我需要你帮我做成动态的，那种循环从左到右的动态效果，同时还要每2秒，给整个按钮一个白色的扫光从按钮上扫过



8.同步分析下我现在的挂机系统，我们虽然设定了挂机金币积累上限，但是我不确定逻辑是否正确，需要你帮我做一轮检查确保这个上限效果是对的


需求文档V4.6 设置系统

1.我们需要在游戏中加入一个设置功能，主要是用来控制游戏内的音效和音量的播放

具体逻辑是：
1.玩家点击StarterGui - TopRightGui - Bg - Options - Button这个按钮，打开设置界面（把StarterGui - Options - Bg的Visible属性改成True）
2.玩家点击StarterGui - Options - Bg - Title - CloseButton这个按钮，关闭界面（把StarterGui - Options - Bg的Visible属性改成False）
...
6.每次点击按钮时，都需要给按钮点击的效果，也就是按下缩小松手还原的状态，比如离线奖励弹框的按钮的点击效果，就复用这个效果即可，设置界面的关闭按钮的点击效果也要有这个效果，保持一致

7.如果关闭Bgm播放，就是把游戏内默认的Bgm以及开战后的Bgm都不播放，再开启来后就根据当前状态重新开始播即可
...
概述：现在我们游戏的回收功能，是进入回收状态下，一个兵一个兵去点击回收，我要加一个一键回收功能，点击后，把所有的兵全部收回背包

相关逻辑是：

1.点击Remove按钮后，进入回收状态，这个逻辑不变，但是需要增加一个逻辑是：把StarterGui - MainGui - RemoveAll的Visible属性改成True
...
1.当玩家在游戏内游戏时，会有各个地方产出金币。
2.当每次产出金币时，需要做出判断：当前服务器内（注意一定是同服务器！！！）是否有玩家的好友正在游戏，如果有，则产出金币时需要有金币加成
    a.有1个好友，金币产出+20%
    b.有2个好友，金币产出+40%
    c.有3个好友，金币产出+60%
    d.有4个好友，金币产出+80%
    ...
    注意：我们VIP也会带来金币加成，VIP的加成要算在这个好友加成计算完后再加成，比如VIP加50%，好友加20%，基础产出是10，那最终的产出=10*（1+20%）*（1+50%）

不论是任何渠道产出金币时，比如挂机/战斗产出的都算，以及七日登录奖励给的金币奖励，也算
注意：只是金币产出给加成，其他不给
...
再强调一下：每次产出金币时：去根据当前的加成值去产出，数值发生变化后下一次产出要精准对上。
另外当计算完出现小数后，要向上取整，比如34.3，也算成35


需求文档V5.9 付费复活功能

概述：游戏内目前玩家所有兵死亡后弹出结算弹框玩家和兵会重生在基地，我们加个功能：玩家付费可以让所有兵复活立刻再次战斗

详细规则：

1.有三个开发者道具：
    a.3517019111,对应在第一章时，购买复活的开发者道具
    b.3517019352,对应在第二章时，购买复活的开发者道具
    c.3517019548,对应在第三章时，购买复活的开发者道具

2.玩家在战斗过程中，如果兵种全部死完了，但是敌方兵还没死完，在弹出结算界面时，显示Confirm按钮时，需要同步把StarterGui - Victory - Information - Revive按钮也显示出来
3.点击Revive按钮，触发该章节对应的复活开发者道具购买
4.玩家购买成功后：
    a.立刻关闭结算界面
    b.玩家布局的所有兵种，在当前场景的IdleFloor对应位置原地复活
    c.复活后立刻开始寻敌战斗，并继续战斗流程

5.有一些需要注意的是：如果玩家通关，也就是打完了最后一关敌方单位全部死完，那么复活时是不显示Revive按钮的
6.只有前三章会有复活功能，第四章玩家死亡后，是不显示复活按钮的，所以这个复活功能是针对前三章的功能
...
概述：使用这个功能后，玩家会进入俯视角，锁定兵种的位置，进入快速编辑的阶段

详细规则：
1.StarterGui - MainGui - Edit按钮，是我们的编辑功能触发按钮，和Remove按钮一样，需要在玩家站在idleFloor上时才显示出来
2.玩家点击Edit按钮后，立刻隐藏Remove按钮，并且把EditExit按钮显示出来，这俩按钮的父节点都是MainGui，和Edit按钮是同层级的
3.玩家点击Edit按钮后，立刻进入俯视角状态，此时玩家无法移动无法跳跃，只能站在原地，并且需要把所有玩家的模型隐藏起来（注意，别人依然能看见，但是玩家自己看不到自己，也就是只是玩家自己的客户端隐藏起来，把所有玩家都隐藏起来）
4.镜头俯视角对准Idlefloor，保持这种状态，高度参数留出来，让我来调整高度
5.在俯视角情况下，玩家可以对兵种进行拖动/合成，由于Remove按钮隐藏了起来，所以暂时不用管移除兵种的操作
...
注意：在接下来的多个需求中，我将会对我的游戏进行大幅度的功能修改与优化，你在开发过程中一定要谨慎

重生系统：
概述：相当于一个等级概念，每次重生，均需要消耗指定条件内容，消耗完成后，为玩家带来一些收益，每次重生类似一次升级，详细规则见下方

详细规则：

重生次数：
1.可以理解为重生等级，新账号或者未重生过的老玩家账号，默认重生次数都是0
2.每次重生，重生次数都+1，比如重生了一次，重生数就设定为1，再次重生，重生数就设定为2
3.每次重生都需要读取配置中的重生条件，只有这一次的重生要求的所有条件都满足，才可以进行重生

重生条件类型：
1.玩家当前拥有的金币数达到指定数值：比如1级的重生要求的条件是10000金币，那重生时玩家的当前的金币数必须大于10000
2.指定id的兵种数量要有x个，比如10001这个兵要求有3个，注意：不论是摆放在地板上还是在背包中未放置的这个兵都算

重生奖励类型：
1.为玩家发放X个指定Id的兵种：比如发放5个10002的兵
2.为玩家发放XXXX金币：比如为玩家发放5000金币
3.让玩家的全局金币产出倍率提升至xx倍，比如我配置了0.2，就是让玩家的金币获取效率提升至1.2倍，比如基础产出是10，那么提升至1.2倍就是每次产出12点，如果配置2.5，那么产出倍率就是3.5，基础产速变成35
    3.1这里有个需要注意的点：我们的VIP/好友等都会带来产速加成，这里的统一逻辑是：最终的金币产出效率 =基础产出*（ 1+重生奖励产出倍率+VIP提供的倍率+好友加成倍率）
    3.2这个金币产出倍率作用于：挂机奖励产出速度/每次击杀兵种时获得的金币，这俩会随着倍率提升而变化，但是在线奖励/七日登录奖励这种一次性给的奖励，不受倍率影响，比如某个奖励投放1000点金币，不论倍率是多少，最终都是领取1000点金币
    3.3注意表里填的是替换制，比如1级填1.2，2级填1.4，那么到了2级生效的是1.4
4.全局攻击力加成：是让我的所有的兵种，直接在自身攻击力的基础上，带来一个额外的百分比攻击力加成，比如加成倍率是1.3，就是所有兵攻击力乘1.3，注意表里填的是替换制，比如1级填1.2，2级填1.4，那么到了2级生效的是1.4.注意我表里填0.2代表加20%，实际是攻击力=基础攻击力*1.2
5.商店解锁新的兵种：
    5.1这是一套对之前规则的修改，我们将会修改为出现在商店中的兵种，也无法购买，需要达到一定的重生次数后才解锁才能购买，不然点了没反应，这部分需求在下面会单独再写
    5.2重生次数达标后，在兵种商店理解解锁这个兵，并且单独对这个兵进行一次库存刷新，还是走库存更新概率逻辑这个不变
    5.3解锁的这个兵解锁后立即执行了一次单独更新库存后，进入整体的库存更新逻辑，在下一次整体库存更新后参与共同刷新
6.解锁新的监狱：
    6.1目前的监狱是根据通关主线关卡来默认解锁的，现在我们需要移除这个逻辑，修改为根据重生次数来解锁
    6.2我在重生表中每个重生次数后都填写了解锁的监狱，如果触发了新的监狱解锁，则立刻解锁新监狱
    6.3监狱自身的带来的挂机速度和时长逻辑保持不变

重生结果：
1.立即清除玩家所有的金币重置为0
2.立刻清楚玩家所有的兵种，包括背包的和地上的
3.注意：在重生后会发放一些奖励，这里的逻辑应该是：先立刻重生重置金币与兵，然后再为玩家发放奖励，千万不要做成我发放了然后又被重置了

客户端逻辑：
1.玩家点击StarterGui - MainGui - Rebirth按钮，打开重生界面（把StarterGui - Rebirth - Bg的visible属性改成True，注意打开界面时走现在通用的弹框打开的动效和Lighting下的Blur，这部分逻辑保持一致）
2.玩家点击StarterGui - Rebirth - Bg - Title - CloseButton按钮，关闭重生界面（把StarterGui - Rebirth - Bg的visible属性改成false。关闭时也要走通用的关闭动效效果）
3.StarterGui - Rebirth - Bg - Rebirthinfo - RewardBg是用于展示重生后的奖励内容的，其中：
    3.1Rebirthinfo - RewardBg - ItemTemplate是奖励内容模板，这个ItemTemplate的默认的Visible属性是False，当弹框内容生成时需要去复制一份模板，更换成对应的奖励信息，并显示出来
    3.2Rebirthinfo - RewardBg - ItemTemplate - ItemIcon是这个奖励的icon，如果是金币就用金币图标，资源id是：rbxassetid://92295649647469；如果奖励是兵种，则需要去兵种表中找这个兵的头像icon资源id，如果奖励是监狱，如果是已经解锁的监狱，这个奖励就不显示，但是如果下一个监狱是新监狱就要显示出来，去HouseConfig中找这个监狱（house）的图标，如果奖励是攻击力加成，则使用图标资源rbxassetid://76416297088295，如果是金币数值加成，则使用rbxassetid://93607334203626
    3.3Rebirthinfo - RewardBg - ItemTemplate - Num是奖励数量，如果是金币，就显示金币数值，如果是兵种，就显示发放的这个兵的数量，如果是新监狱解锁，则不显示Num，如果是攻击力加成，则显示加成数值，注意：我填的数值是0.2的话，实际加了20%，但是这里数值应该显示为x1.2,如果是金币产出加成，这里的逻辑也是一样的，和攻击力加成比例显示规则是一致的
    3.4Rebirthinfo - RewardBg - ItemTemplate - Name是奖励名字，如果是金币，不显示，如果是兵种，也不显示，如果是新监狱，这里名字统一叫：New！，如果是攻击力加成或者金币产出加成，也不显示
4.StarterGui - Rebirth - Bg - Rebirthinfo - SpendBg是用于展示要求的消耗内容的，其中：
    4.1Rebirthinfo - RewardBg - ItemTemplate是消耗内容模板，这个ItemTemplate的默认的Visible属性是False，当弹框内容生成时需要去复制一份模板，更换成对应的消耗内容信息，并显示出来
    4.2由于我们的消耗只有兵种和金币，金币在这里不显示，所以理论上这里只需要显示对应的消耗的兵种的信息即可，结构与RewardBg - ItemTemplate下的结构是一致的，去找这个兵对应的头像展示出来即可，以及数量显示正确以及隐藏名字
    4.3Rebirthinfo - SpendBg - ItemTemplate - Num格式是x/y,x是当前有的数量，y是要求的数量，如果x小于y，x需要是红色，如果大于等于了，就是默认颜色
5.StarterGui - Rebirth - Bg - Rebirthinfo - ProgressBg是用于展示本次重生需要的金币的玩家当前的获得进度条
    5.1StarterGui - Rebirth - Bg - Rebirthinfo - ProgressBg - Progress是进度条，金币数量是0的情况下，Progress的Size的X的Scale数值是0,如果金币数量达到了要求值，进度条满，就是把Progress的Size的X的Scale数值改成0.998
    5.2StarterGui - Rebirth - Bg - Rebirthinfo - ProgressBg - Progress - Num是显示金币数值要求情况，格式是x/y,x是当前有的金币数，格式是$xxx，y是要求的数值，格式是$yyy，注意双方的数值都不显示大数值，显示出具体的格式，但是如果数字很大，要显示出西方数字显示的逗号，比如10000，应该显示为10,000
6.StarterGui - Rebirth - Bg - Rebirthinfo - RebirthBtn是重生按钮，玩家点击后，如果条件满足，则点击后触发重生

重生交互表现：
1.如果玩家重生要求的内容未达成，则点击RebirthBtn按钮无反应，并且点击时要播放现在的警告音
2.如果玩家条件满足，则重生成功：
    2.1立刻关闭当前的重生弹框，然后再立刻打开，再打开后，弹框里的内容就变成了下次重生所需要的内容和奖励
    2.2重生完成后，需要立刻弹出tips，也就是StarterGui - TipsSystem - RebirthTips的visible改成true（显示出来），显示出来时需要有弹出动画，也就是从下面一点的位置滑动出来到目标位置，然后停留2秒后再隐藏
    2.3玩家手动关闭了重生界面后，如果本次重生解锁了新的监狱，则需要再播一遍现在的新监狱解锁的表现逻辑（镜头/监狱替换/弹框等）

按钮ui表现：
1.StarterGui - MainGui - Rebirth - Time是textlabel，用于显示玩家的当前的重生次数，格式是[xx]，xx就是当前的重生次数
2.StarterGui - MainGui - Rebirth - RedPoint是红点，如果当前已经满足可重生条件，则需要把RedPoint的visible改成true，同时显示出来后需要给红点一个动效，类似在线或者七日奖励的红点效果

注意：
1.由于我们更改了监狱的解锁条件，与线上当前的条件不同，这二者要兼容，上线后，如果玩家当前的监狱等级已经高于重生后解锁的新的监狱，则不在奖励中显示，重生成功后也不走监狱解锁替换表现逻辑
2.线上未来的新监狱的解锁条件全部变成由重生次数控制
3.由于兵种商店加入了解锁条件，这里更新后可以直接生效
4.当达到最高级的重生次数后，再点击重生按钮，直接点击没反应。同时最后一次重生后，关闭弹框后就不再打开弹框了，如果后续版本更新了新的重生次数，再点击按钮可以弹出弹框即可

关于兵种商店解锁的客户端表现：
1.如果商店中这个兵未解锁，则需要把ArmyStore - StoreBg - ScrollingFrame - ItemCardTemplate - UnlockBg和ArmyStore - StoreBg - ScrollingFrame - ItemCardTemplate - UnlockText显示出来，这俩默认是隐藏的
2.ArmyStore - StoreBg - ScrollingFrame - ItemCardTemplate - UnlockText的文本内容需要改成You Need x Rebirths!,其中x是解锁需要的重生次数


下面是我的重生相关的数据表：
重生次数	所需金币	所需兵种	所需数量	奖励金币	奖励的兵	奖励兵的数量	金币产出倍率	攻击力全局加成	商店解锁新兵种	解锁监狱
1	1000	10001	5	500	10002	3	0.2	0.2	10005	PrisonLv2
2	2000	10002	5	1000	10003	3	0.4	0.4	10006	PrisonLv2
3	3000	10003	5	2000	10004	3	0.6	0.6	10007	PrisonLv2
4	4000	10004	5	3000	10005	3	0.8	0.8	10008	PrisonLv2
5	5000	10005	5	5000	10006	3	1	1	10009	PrisonLv3
6	6000	10006	8	8000	10007	3	1.2	1.2	10010	PrisonLv3
7	7000	10007	8	12000	10008	3	1.4	1.4	10011	PrisonLv3
8	8000	10008	8	15000	10009	3	1.6	1.6	10012	PrisonLv3
9	9000	10009	8	20000	10010	3	1.8	1.8	10013	PrisonLv3
10	10000	10010	8	25000	10011	3	2	2	10014	PrisonLv4
11	11000	10011	12	30000	10012	3	2.2	2.2	10015	PrisonLv4
12	12000	10012	12	40000	10013	3	2.4	2.4	10016	PrisonLv4
13	13000	10013	12	50000	10014	3	2.6	2.6	10017	PrisonLv4
14	14000	10014	12	60000	10015	3	2.8	2.8	10018	PrisonLv4
15	15000	10015	12	70000	10016	3	3	3	10019	PrisonLv4
16	16000	10016	15	80000	10017	3	3.2	3.2	10020	PrisonLv4
17	17000	10017	15	90000	10018	3	3.4	3.4	10021	PrisonLv4
18	18000	10018	15	100000	10019	3	3.6	3.6	10022	PrisonLv4
19	19000	10019	15	110000	10020	3	3.8	3.8	10023	PrisonLv4
20	20000	10020	15	120000	10021	3	4	4	10024	PrisonLv4


需求文档V7.1 地图系统（或者叫章节选择系统）

概述：
我们之前的章节是一章通关完后下次自动到下一章，我们需要修改成玩家能够自己选择去哪个章节战斗

详细规则：
1.新玩家默认的基础关卡是第一章，第一章默认解锁
2.后续的其他章节，均需要玩家使用“勋章”进行解锁，也就是每个章节都会配置一个解锁所需要的“勋章”数量
3.为玩家增加一个货币“勋章”，玩家每次通关某个章节，都可以获得这个章节对应数量的勋章，当玩家累计的勋章数达到关卡要求的数量时，就解锁这个章节
4.为每个章节都配置对应的勋章数量，当玩家的勋章数达到目标值时，自动解锁这个地图，解锁后的地图玩家就可以选中然后去挑战

关于勋章的获得：
1.玩家每次通关了某个地图的最后一关后，都会获得这个地图对应数量的勋章，比如某个地图配置胜利后获得10个勋章，那么玩家通关这章最后一关就会获得10个勋章，可以多次挑战，每次通关成功都会获得一次

关于关卡解锁：
1.勋章数量发生变化时去校验地图的解锁条件，如果达到了新的地图的解锁条件则自动解锁，注意：解锁不会扣除勋章数量，这是个永久累加的数值

关于关卡选择：
1.玩家可以在已经解锁的关卡中任意选择关卡进行挑战，但是无法选择未解锁的关卡

每个新玩家的勋章的默认数量都是0

客户端规则：

1.玩家点击StarterGui - MainGui - BattleControl - Play按钮，之前是直接开始挑战，现在是打开地图界面（把StarterGui - Map - MapBg的Visible改成true，同时StarterGui - Map - Bg也要显示出来），打开时要有游戏现在通用的打开动效和Blur效果
2.玩家点击StarterGui - Map - MapBg - CloseButton按钮关闭地图界面，（把StarterGui - Map - MapBg的Visible改成false，以及StarterGui - Map - Bg的visible也要改成false），同时也要关闭blur和有关闭动效
3.StarterGui - Map - MapBg - ScrollingFrame - MapTemplate是地图模板，常规状态下是默认隐藏的，在地图界面生成时，需要去复制这个模板并显示出来，生成我们对应的地图列表的信息
4.关于复制出来的地图信息的逻辑是这样的：
    4.1StarterGui - Map - MapBg - ScrollingFrame - MapTemplate - MapIcon是imagelabel，用于显示这个地图的icon，我会在地图表中增加地图icon的配置
    4.2StarterGui - Map - MapBg - ScrollingFrame - MapTemplate - MapName是textlabel，用于显示这个地图的名字，在表中会配置
    4.3StarterGui - Map - MapBg - ScrollingFrame - MapTemplate - MedalNum是textlabel，用于显示这个地图（章节）通关后可以获得的勋章数量，格式是*xx，xx就是配置的数量
    4.4某个地图未解锁时，需要把StarterGui - Map - MapBg - ScrollingFrame - MapTemplate - LockBg的visible属性改成True，然后把StarterGui - Map - MapBg - ScrollingFrame - MapTemplate - LockBg - UnlockDes的文本改成xx Medals To Unlock,xx是解锁这个地图需要的勋章数量
    4.5某个地图解锁后，如果玩家点击选择了这个地图，则需要把StarterGui - Map - MapBg - ScrollingFrame - MapTemplate - ChooseBg的visible属性改成True，切换了选中的目标后，再隐藏
    4.6注意：未解锁的地图无法选中
    4.7每次打开地图时，自动选择玩家上一次挑战关卡时候的地图，如果玩家是新玩家，则默认选中第一个地图
5.玩家点击StarterGui - Map - MapBg - AttackButton按钮，立刻关闭地图界面并自动开始挑战当前选中的章节进行战斗，

注意，需要对一些现有规则做一些兼容性处理说明：
1.新的地图需要勋章来解锁，但是线上的老玩家已经解锁了章节的，自动默认解锁，比如线上挑战到了第五章，则进入游戏也默认解锁至第五章，第六章才开始用勋章解锁
...
概述：一个每天utc0点定时刷新的每日任务

详细规则：

任务类型：
1.保持在线时长达到xx分钟：这个要求中间不离线，离线后就要重新开始计算在线时长了
2.今日累计收集xx枚勋章，这个今日多次登录期间累计获得的即可
3.今日累计击败xx个敌人，这个今日多次登录期间累计获得的即可

任务刷新时间：
1.每天的utc0点固定刷新重置任务进度，这个可以做成玩家每次登录时看是否跨过了一个新的utc0点来判断是否刷新任务

任务奖励：
1.现在我们在每日任务中只设定一种奖励类型：就是手铐（也就是限时兵种兑换消耗的那个手铐）

具体的任务内容：
1.我们固定只有三条任务，分别是：
    1.1每天保持在线时长达到15分钟，奖励1个手铐
    1.2每天累计收集20个枚勋章，奖励1个手铐
    1.3每天累计击败100个敌人，奖励1个手铐

详细的客户端规则：
1.玩家点击StarterGui - MainGui - Daily按钮，可以打开每日任务界面（把StarterGui - DailyTask - Bg的visible属性改成true，注意打开弹框和关闭弹框都要有我们游戏内现有的弹框动效，同时也要开启blur)
2.玩家点击StarterGui - DailyTask - Bg - Title - CloseButton按钮，关闭每日任务界面
3.当玩家完成了任务，有可领取但是未领取的奖励时，需要把任务按钮上的红点显示出来（把StarterGui - MainGui - Daily - RedPoint显示出来，同时显示出来的时候要有动效）
4.StarterGui - DailyTask - Bg - TaskBg - Task01对应第一条任务，其中：
    4.1Task01 - TaskProgress是进度描述，格式是（x/y),x代表当前值，y代表目标值，当任务未完成时，x是红色，Y是这个textlabel默认的颜色，如果任务完成，文本都变成绿色
    4.2当任务未完成时，需要把Task01 - Claim的BackgroundColor改成162, 162, 162，同时把Task01 - Claim - UIGradient的Enable改成false，此时点击按钮，要有默认的警告音，点了无法领取奖励
    4.3当任务完成后可以领取奖励了，需要把Task01 - Claim的BackgroundColor改成255，255，255，然后把Task01 - Claim - UIGradient的Enable改成true，这时候点击claim按钮可以领取奖励
    4.4成功领取奖励时，需要先暂时隐藏任务界面，然后弹出弹框，走我们默认通用的领取奖励成功的弹框，比如登录奖励或者在线奖励领取成功时的表现弹框，关闭弹框后，再把任务界面显示出来
    4.5当某个任务奖励被领取时，需要把Task01 - Claim隐藏，同时把Task01 - Claimed显示出来

    注意：Task02和Task03是与Task01一样的结构，可以按照一样的逻辑来实现
