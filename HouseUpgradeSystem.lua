...
function HouseUpgradeSystem.ReplaceHouseModel(player, newModelName)
	...
	-- 获取当前房屋模型的位置（使用模型的轴点位置）
	local currentModelCFrame
	if currentHouseModel.PrimaryPart then
		currentModelCFrame = currentHouseModel.PrimaryPart.CFrame
	else
		-- 如果没有PrimaryPart，使用模型的根部位置
		currentModelCFrame = currentHouseModel:GetPivot()
	end

	print(string.format(
		"[HouseUpgradeSystem] 当前房屋 %s 轴点位置: %s",
		currentHouseModel.Name,
		tostring(currentModelCFrame.Position)
	))
	...
	-- 直接使用当前房屋的轴点位置作为新房屋的位置
	-- 不进行任何偏移计算，严格以模型的轴点放置
	if newHouseModel.PrimaryPart then
		newHouseModel:SetPrimaryPartCFrame(currentModelCFrame)
	else
		newHouseModel:PivotTo(currentModelCFrame)
	end

	print(string.format(
		"[HouseUpgradeSystem] 新房屋 %s 已放置在轴点位置: %s",
		newModelName,
		tostring(currentModelCFrame.Position)
	))
